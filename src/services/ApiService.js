...
const SERVICE = new Vuex.Store({
  actions: {
    ...
    generateTokenLaravel () {
      Axios.get(env.api_log + '/generate-token')
        .then(r => {
          ...
          if (r.data.token != undefined) Axios.defaults.headers.common['X-Access-Token'] = Storage.get('data_token')
    ...
    validateReset ({commit}, {self}) {
      ...
      Axios.get(env.api_log + '/active-directory/validate-reset')
        .then((r) => {
          if (r.status === 200) {
    ...
    searchText ({commit}, {self}) {
      Util.openLoadModal(self)
      self.new_params = {}
      Axios.get(env.api_log + '/active-directory/search', {params: self.params})
        .then(r => {
          if (r.status === 200) {
            Util.closeLoadModal(self)
            self.dataAlert = {}
    ...
    unlock ({commit}, {self}) {
      Util.openLoadModal(self)
      self.new_params = {}
      Axios.get(env.api_log + '/active-directory/unlock', {params: self.params})
        .then(r => {
          if (r.status === 200) {
            let msg = r
    ...
    reset ({commit}, {self}) {
      ...
      Axios.post(env.api_log + '/unlockresetuser/resetpassword', self.params)
        .then(r => {
          if (r.status === 200) {
            Axios.get(env.api_log + '/unlockresetuser/search/' + self.params.username)
              .then((rpta) => {
                if (rpta.status === 200) {
                  Util.closeLoadModal(self)
                  self.dataAlert = r
